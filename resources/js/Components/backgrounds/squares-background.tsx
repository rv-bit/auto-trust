...
const Squares: React.FC<SquaresProps> = ({
	...
	const resolvedBorderColor = useRef<string>("");
	const resolvedHoverColor = useRef<string>("");
	...
	const resolveColor = useCallback(
		(color: string): string => {
			...
			if (color.startsWith("var(")) {
				...
				if (varName && canvasRef.current) {
					const computedColor = getComputedStyle(canvasRef.current).getPropertyValue(varName).trim();
	...
	useEffect(() => {
		...
		const resizeCanvas = () => {
			...
			resolvedBorderColor.current = resolveColor(borderColor as string);
			resolvedHoverColor.current = resolveColor(hoverFillColor as string);
			...
			for (let x = startX; x < canvas.width + squareSize; x += squareSize) {
				for (let y = startY; y < canvas.height + squareSize; y += squareSize) {
					...
					if (hoveredSquareRef.current && Math.floor((x - startX) / squareSize) === hoveredSquareRef.current.x && Math.floor((y - startY) / squareSize) === hoveredSquareRef.current.y) {
						...
						ctx.globalAlpha = hoverColorOpacity ?? 1.0;
						ctx.fillRect(squareX, squareY, squareSize, squareSize);
					}

					ctx.strokeStyle = resolvedBorderColor.current;
					ctx.globalAlpha = borderColorOpacity ?? 1.0;
	...
	return <canvas ref={canvasRef} className={cn("block h-full w-full border-none", className)}></canvas>;
};
