...
function SidebarProvider({
	...
	const setOpen = React.useCallback(
		(value: boolean | ((value: boolean) => boolean)) => {
			...
			document.cookie = `${CHAT_SIDEBAR_COOKIE_NAME}=${openState}; path=/; max-age=${CHAT_SIDEBAR_COOKIE_MAX_AGE}`;
		},
		[setOpenProp, open],
	);
...
function Sidebar({
	...
	return (
		...
		>
			...
			<div
				className={cn(
					"relative h-svh w-(--sidebar-width) bg-transparent transition-[width] duration-200 ease-[cubic-bezier(0.76,0,0.24,1)]",
			...
			<div
				className={cn(
					"fixed inset-y-0 z-10 hidden h-svh w-(--sidebar-width) transition-[left,right,width] duration-200 ease-[cubic-bezier(0.76,0,0.24,1)] md:flex",
					side === "left"
			...
			>
				<div
					data-sidebar="sidebar"
					className="bg-sidebar group-data-[variant=floating]:border-sidebar-border flex h-full w-full flex-col group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:shadow-sm gap-5"
				>
					{children}
				</div>
			</div>
...
function SidebarInset({ className, children, ...props }: React.ComponentProps<"div"> & { children: React.ReactNode }) {
	return (
		<div
